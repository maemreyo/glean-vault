---
created: 2025-12-16
tags: vocabulary, flashcard
category: word
confidence:
mastery: 🔴 New
---
vault
# vault

## 📊 Meta Information

| Property | Value |
|----------|-------|
| **IPA** | /vɔːlt/ |
| **Part of Speech** | noun, verb |
| **CEFR Level** | B2 |
| **Frequency** | common |
| **Register** | neutral, formal |
| **Etymology** | Old French "voute" from Vulgar Latin "*volvita*" "arched vault", from Latin "*volvitus*" past participle of "*volvere*" "to roll, turn" |

---

## 🌳 Root Word Analysis

### Root Components
**Root:** Latin "*volvere*" = "to roll, turn, revolve"
**Prefix:** (Không có tiền tố)
**Suffix:** (Không có hậu tố đặc biệt)

### Word Family (Họ từ cùng gốc)
- **Noun:** vault, vaulting
- **Verb:** vault
- **Adjective:** vaulted
- **Adverb:** (Không có)

### Related Words (Same Root)
> Các từ khác cùng gốc:
- [[involve]] - bao gồm, liên quan đến (in- + volvere)
- **revolve** - xoay vòng, quay quanh (re- + volvere)
- **volume** - thể tích, cuốn sách (vol- + ume, từ "roll" - cuộn)
- **convoluted** - phức tạp, rối rắm (con- + voluted)
- **devolve** - chuyển giao, ủy thác (de- + volvere)

---

## 💡 Definitions

### Root Meaning (Nghĩa gốc trong từ điển)
1. **(noun)** A room or compartment, often built of stone or brick, used for storing valuables
2. **(noun)** An arched or dome-shaped roof or ceiling
3. **(verb)** To leap or jump, especially using the hands or a pole

### Context Meaning (Nghĩa trong ngữ cảnh cụ thể)
1. **Banking/Security:** Két sắt, hầm chứa tiền, kho bảo mật
2. **Architecture:** Mái vòm, trần vòm, arch
3. **Sports/Gymnastics:** Nhảy xà, nhào lộn, leap over

### Vietnamese Translation
- **Danh từ:** hầm, két sắt, kho, mái vòm, trần vòm
- **Động từ:** nhảy qua, vượt qua (bằng cách bật lên)

### Nuances (Sắc thái nghĩa)
> Các nghĩa khác nhau tuỳ ngữ cảnh:
- **Storage vault:** Két sắt, kho chứa (cho tiền, tài liệu, đồ giá trị)
- **Cathedral vault:** Mái vòm (trong kiến trúc nhà thờ Gothic)
- **Vaulted ceiling:** Trần vòm (kiến trúc, tạo cảm giác rộng rãi)
- **To vault over:** Nhảy qua một vật cản
- **Pole vault:** Nhảy xà (môn thể thao)

---

## 🔗 Relations

### Synonyms (Đồng nghĩa)

| Word | Meaning | Usage Difference |
|------|---------|------------------|
| **safe** | két sắt, an toàn | Thường nhỏ hơn vault, dùng trong nhà, cá nhân |
| **strongroom** | phòng bảo mật | Tương tự vault nhưng thường trong ngân hàng |
| **chamber** | phòng, buồng | Tổng quát hơn, không nhất thiết để chứa đồ giá trị |
| **arch** | vòm | Chỉ cấu trúc vòm, không phải không gian bên dưới |
| **leap** | nhảy | Tổng quát hơn vault, không cần dùng tay hoặc xà |
| **jump** | nhảy | Nhảy thông thường, không có kỹ thuật như vault |

### Antonyms (Trái nghĩa)

| Word | Meaning | Usage Difference |
|------|---------|------------------|
| **expose** | phơi bày, lộ ra | Trái ngược với bảo mật của vault |
| **reveal** | tiết lộ | Đối lập với việc giấu giếm trong vault |
| **open** | mở ra | Trái với đóng kín của vault |
| **lower** | hạ xuống | Đối lập với "vault" là nhảy lên, vươn lên |
| **descend** | đi xuống | Ngược lại với động tác nhảy lên cao của vault |

---

## 📝 Usage

### Collocations (Cụm từ đi kèm)

| Phrase | Meaning | Frequency |
|--------|---------|-----------|
| **bank vault** | két sắt ngân hàng | very common |
| **vault door** | cửa két sắt | common |
| **vaulted ceiling** | trần vòm | common |
| **pole vault** | nhảy xà (thể thao) | very common |
| **safety vault** | hầm an toàn, hầm tránh | common |
| **vault into** | nhảy vào | uncommon |
| **vault over** | nhảy qua | common |
| **burial vault** | quan tài đá, hầm mộ | uncommon |
| **cloud vault** | kho lưu trữ đám mây | common (IT) |

### Examples (Ví dụ câu)

#### Example 1
**Sentence:** The bank robbers couldn't break into the vault because it had a 12-inch steel door.
**Translation:** Cướp ngân hàng không thể đột nhập vào két sắt vì nó có cửa thép dày 12 inch.
**Context:** Banking/Security - describing the security features of a bank

#### Example 2
**Sentence:** The cathedral's Gothic architecture featured magnificent ribbed vaults that soared 50 meters above the ground.
**Translation:** Kiến trúc Gothic của nhà thờ có những mái vòm xương sườn tuyệt vời vươn cao 50 mét trên mặt đất.
**Context:** Architecture - describing cathedral construction

#### Example 3
**Sentence:** The athlete easily cleared the bar at 5.5 meters in the pole vault competition.
**Translation:** Vận động viên dễ dàng vượt qua xà ở độ cao 5.5 mét trong cuộc thi nhảy xà.
**Context:** Sports/Gymnastics - track and field event

### Common Mistakes (Lỗi sai phổ biến)

| Mistake ❌ | Correction ✅ | Explanation |
|-----------|--------------|-------------|
| "The treasure is in the vaulted" | "The treasure is in the vault" | "Vaulted" là tính từ, "vault" mới là danh từ chỉ kho chứa |
| "He vaulted the wall with a rope" | "He vaulted over the wall" | "Vault" khi là động từ cần giới từ "over/into/onto" để chỉ hướng |
| "The ceiling has vaults" | "The ceiling has vaulting/is vaulted" | "Vault" không diễn tả cấu trúc trần, dùng "vaulting" hoặc "vaulted" |

---

## 🎯 Learning Tips

### Memory Techniques (Kỹ thuật ghi nhớ)
1. **Visual:** Hình ảnh một cái két sắt khổng lồ (vault) bên trong ngân hàng
2. **Connection:** Liên kết với "volved" trong "involve" - đều có gốc "roll/turn"
3. **Sound Association:** "Vault" nghe giống "vòm" trong tiếng Việt
4. **Action:** Tưởng tượng một vận động viên nhảy xà (pole vault) lên cao

### Practice Exercises (Bài tập)
1. Điền vào chỗ trống: The bank keeps its money in a secure _______.
2. Viết 3 câu với "vault" theo 3 nghĩa khác nhau
3. Tìm 5 từ đồng nghĩa với "vault" trong nghĩa "két sắt"

### Related Concepts (Khái niệm liên quan)
- **Architecture:** arch, dome, ceiling, Gothic, Romanesque
- **Security:** safe, strongroom, lock, security system
- **Sports:** gymnastics, athletics, pole vault, high jump

---

## 🧩 Context (Nguồn gốc câu gặp)

> [!quote] Original Sentence
>
> — Source: [[]]
> — Timestamp:

**Giải thích ngữ cảnh:**
[]

---

## 🧠 Spaced Repetition Flashcards

### Card 1: Recognition (Nhận biết)
Question:: What does **vault** mean?
Answer:: A secure room for storing valuables OR an arched roof OR to jump over something

### Card 2: Production (Sản xuất)
Question:: Từ tiếng Anh nào có nghĩa là "két sắt" hoặc "mái vòm"?
Answer:: **vault**

### Card 3: Usage (Sử dụng)
Question:: Use **vault** in a sentence about banking
Answer:: The bank's vault contains millions of dollars in cash and gold.

### Card 4: Collocation (Cụm từ)
Question:: What are 3 common collocations with **vault**?
Answer:: bank vault, pole vault, vaulted ceiling

### Card 5: Root Analysis (Phân tích gốc)
Question:: What is the root of **vault** and what does it mean?
Answer:: Latin "volvere" meaning "to roll, turn"

---

## 📈 Learning Progress

**Times encountered:** 1
**Times reviewed:** 0
**Confidence level:** 🔴 New → 🟡 Learning → 🟢 Familiar → 🔵 Mastered
**Next review:**

---

## 💭 Personal Notes & Mnemonics

### Visual Association (Liên tưởng hình ảnh)
- Kết hợp 3 hình ảnh: két sắt ngân hàng, mái vòm nhà thờ, vận động viên nhảy xà

### Story/Situation (Câu chuyện ghi nhớ)
The athlete took a running **vault** over the barrier to reach the bank's **vault** with its impressive **vaulted** ceiling.

### Personal Connection (Kết nối cá nhân)
- Liên tưởng với "vault" trong Fallout (game) - hầm trú ẩn an toàn

---

## 🔖 Tags & Categories

**Themes:** #architecture #security #sports
**Topics:** #banking #construction #athletics #gymnastics
**Difficulty:** #intermediate #B2
**Source Type:** #article #book