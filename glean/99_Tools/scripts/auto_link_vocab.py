...
def convert_html_tables_in_content(content):
    """Convert all HTML tables in content to Markdown format."""
...
def process_file(filepath, terms_map, sorted_term_keys, dry_run=True):
    ...
    # Step 1: Convert HTML tables to Markdown (if any)
    converted_content, table_count = convert_html_tables_in_content(content)
    if table_count > 0:
        if not dry_run:
    ...
    for line in lines:
        # Check if line is a table row (Markdown table with |)
